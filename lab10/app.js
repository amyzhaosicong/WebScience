...
// write calendar csv data
app.post('/calendarCSV', function(req, res) {
	console.log("inside calendarCSV");
	var csv = req.body.csv;
	console.log(csv);
	fs.writeFile("calendar.csv", csv, function(err) {
		if (err) throw err;
		console.log('calendar csv saved');
	});
});

// write map json data
app.post('/mapJSON', function(req, res) {
	console.log("inside mapJSON");
	var json = req.body.json;
	console.log(json);
	fs.writeFile("map.json", JSON.stringify(json,null,4), function(err) {
		if (err) throw err;
		console.log("map json saved");
	});
});


// connect to the mongodb
MongoClient.connect("mongodb://localhost:27017/lab1", function(err, db) {
